...
class RedisManager:
    ...
    async def initialize(self):
        ...
        async with self._lock:
            ...
            try:
                ...
                log.info(
                    f"Redis connection established to database {
        self._database_index} with unified cache"
                )
    ...
    async def get_credential_state(self, filename: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if credential_entry and "state" in credential_entry:
                log.debug(
                    f"Retrieved credential state from unified cache: {filename} in {
        operation_time:.3f}s"
                )
    ...
    async def get_all_credential_states(self) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            log.debug(
                f"Retrieved all credential states from unified cache ({
        len(states)}) in {
            operation_time:.3f}s"
            )
            return states
    ...
    async def get_usage_stats(self, filename: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if credential_entry and "stats" in credential_entry:
                log.debug(
                    f"Retrieved usage stats from unified cache: {filename} in {
        operation_time:.3f}s"
                )
    ...
    async def get_all_usage_stats(self) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            log.debug(
                f"Retrieved all usage stats from unified cache ({
        len(stats)}) in {
            operation_time:.3f}s"
            )
            return stats
